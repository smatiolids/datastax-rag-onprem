...
############################
import cassio
session = getCQLSession("dse")
cassio.init(session=session, keyspace=os.environ["KEYSPACE"])
...
# Function for Vectorizing uploaded data into DSE
...
def load_vectorstore(username):
    # Get the load_vectorstore store from DSE
    return Cassandra(
        embedding=embedding,
